...
const PostProcessing = ({
  ...
  // Ensure size is fully initialized before setting up post-processing
  useEffect(() => {
  ...
  const render = useMemo(() => {
    if (!gl || !isReady || size.width === 0 || size.height === 0) return null;
    ...
    try {
      ...
      console.warn('WebGPU PostProcessing initialization error, falling back:', e);
      return null;
    }
  }, [camera, gl, scene, strength, threshold, fullScreenEffect, isReady, size.width, size.height]);

  useFrame(({ clock }) => {
    if (!render || size.width === 0 || size.height === 0) return;
    try {
      // Animate the scan line from top to bottom
      progressRef.current.value = Math.sin(clock.getElapsedTime() * 0.5) * 0.5 + 0.5;
      render.render();
    } catch {
      // Prevent unhandled command buffer crashes on rapid resizing
    }
  }, 1);
...
export const Html = () => {
  ...
  return (
    <div className="h-svh relative w-full overflow-hidden bg-black select-none">
