...
## Target Host & Connection Settings Resolution

When generating `ansible/hosts.ini`, the compiler determines the target host and user configuration based on the visual layout of the canvas:

1. **Topologically Connected Parent VM**: If the starting Ansible node has an incoming connection from a VM node (e.g. EC2 instance, GCP compute instance, or Azure VM), the compiler automatically binds `ansible_host` to reference the parent's IP address (e.g. `aws_instance.web_server.public_ip`).
2. **Custom Target Host IP**: If the starting Ansible node is disconnected from any VM nodes but has a custom target IP or variable reference configured in the `Target Host IP` (`ansibleHost`) panel, the compiler writes this value directly.
3. **VM Canvas Fallback**: If no incoming VM edge is present, but an EC2/GCP VM node exists on the canvas, the compiler falls back to mapping its IP output for backwards-compatibility.
4. **Local Host Fallback**: If no VM nodes are found on the canvas, the compiler falls back to targeting `127.0.0.1`.

### Sandbox Redirection
During local sandbox execution (`isSandboxRun && isDocker`), the Go runner automatically intercepts the resolved host IP matching AWS/GCP/Azure/variable patterns and redirects execution safely to the local container `ubuntu_ssh_1`.
