...
### 2. `aws_subnet`
- Dynamic reference to parent VPC (`vpc_id = aws_vpc.main.id`). Supports manual override via `vpcId` parameter, incoming VPC node edges, or fallback first VPC discovery.
...
- Dynamic binding to security groups (`vpc_security_group_ids`). Accepts manual override parameter `securityGroupId`, incoming Security Group edges, or fallback discovery.
...
### 6. `aws_db_instance` (RDS PostgreSQL)
- Engine selection (`postgres`).
...
- Dynamic network binding: Accepts manual input for `vpcSecurityGroupIds` and `dbSubnetGroupName`, incoming Security Group edges, or default mock parameters.
