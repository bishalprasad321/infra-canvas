...
The key architectural point: the **Runner still terminates SSH and still does the log-pattern scanning it does today** (see [[03.4 - Live Node Status Tracking & Stream Scanning]]). Only the transport underneath the SSH connection changes — a TCP socket becomes a multiplexed tunnel stream. `runner.go`'s existing SSH client code does not need to be rewritten, only re-pointed at a `net.Conn` implementation backed by the tunnel instead of a direct dial.
